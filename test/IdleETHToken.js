...
contract('IdleETH', function ([_, creator, nonOwner, someone, foo]) {
  ...
  //   await this.ETHMock.transfer(this.cETHMock.address, BNify('2500000000000000000'), { from: creator });
  ...
  //   // when redeeming now we redeem more ETH of what cETHMock has so we transfer ETH to the contract
  //   await this.ETHMock.transfer(this.iETHMock.address, BNify('1').mul(this.one), { from: creator });
  //
  //   const res1 = await this.token.tokenPrice.call();
  ...
  //   // tokenPrice is 1 here
  //   await this.mintIdle(BNify('10').mul(this.one), nonOwner);
  //   // so 10 ETH will be transferred from nonOwner
  //   const resBalanceETH = await this.ETHMock.balanceOf.call(nonOwner, { from: nonOwner });
  //   resBalanceETH.should.be.bignumber.equal(BNify('0').mul(this.one));
  //   // so 10 IdleETH will be minted to nonOwner
  //   const resBalanceIdle = await this.token.balanceOf.call(nonOwner, { from: nonOwner });
  //   resBalanceIdle.should.be.bignumber.equal(BNify('10').mul(this.one));
  //   // and 500 cETH will be minted to IdleETH contract
  ...
  //   const resBalanceETH2 = await this.ETHMock.balanceOf.call(nonOwner, { from: nonOwner });
  ...
  //   await this.ETHMock.transfer(nonOwner, BNify('10').mul(this.one), { from: creator });
  //   await this.ETHMock.approve(this.token.address, BNify('10').mul(this.one), { from: nonOwner });
  ...
  //   const resBalanceETH = await this.ETHMock.balanceOf.call(nonOwner, { from: nonOwner });
  //   resBalanceETH.should.be.bignumber.equal(BNify('10').mul(this.one));
  // });
  ...
  //   await this.ETHMock.transfer(this.cETHMock.address, BNify('1250000000000000000'), { from: creator });
  //   // 4 * 1.3 = 5.2 ETH nav of iETH pool
  //   // so we transfer 1.2 ETH to iETH mock to cover new interests earned
  //   await this.ETHMock.transfer(this.iETHMock.address, BNify('1200000000000000000'), { from: creator });
  ...
  //   const resBalanceETH3 = await this.ETHMock.balanceOf.call(nonOwner, { from: nonOwner });
  //   resBalanceETH3.should.be.bignumber.equal(BNify('11450000000000000000')); // 11.45
  // });
  ...
  //   await this.ETHMock.transfer(this.iETHMock.address, BNify('2').mul(this.one), { from: creator });
  //
  //   // set same rates so to use _calcAmounts from IdleRebalancer
  //   await this.cETHWrapper._setAPR(BNify('2200000000000000000')); // 2.2%
  //   await this.iETHWrapper._setAPR(BNify('2200000000000000000')); // 1.1%
  ...
  //   // so _rebalanceCheck would return true
  //
  //   await this.ETHMock.transfer(this.token.address, BNify('10').mul(this.one), { from: creator });
  //
