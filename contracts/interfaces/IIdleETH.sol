...
interface IIdleETH {
   ...
   */
  function getParamsForMintIdleToken() external payable returns (address[] memory, uint256[] memory);
   ...
   */
  function rebalance(uint256[] calldata _clientProtocolAmounts) external payable returns (bool);
   ...
   *                   eg [amountCompound, amountFulcrum]
   */
  function getParamsForRebalance() external payable returns (address[] memory, uint256[] memory);
}
