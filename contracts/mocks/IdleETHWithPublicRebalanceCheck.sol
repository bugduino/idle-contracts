...
import "../interfaces/IIdleETH.sol";

import "../IdleRebalancer.sol";
import "../IdleETHPriceCalculator.sol";

contract IdleETHWithPublicRebalanceCheck is ERC20, ERC20Detailed, ReentrancyGuard, Ownable, Pausable, IIdleETH {
  ...
  /**
   * It allows owner to set the IdleETHPriceCalculator address
   *
   * @param _priceCalculator : new IdleETHPriceCalculator address
   */
   ...
   *
   * @param _token : protocol token address (eg. cDAI)
  ...
  function tokenPrice()
    ...
    returns (uint256 price) {
      ...
      price = IdleETHPriceCalculator(priceCalculator).tokenPrice(
   ...
   */
  function mintIdleToken(uint256[] memory _clientProtocolAmounts)
    public payable nonReentrant whenNotPaused whenITokenPriceHasNotDecreased
   ...
   */
  function rebalance(uint256[] memory _clientProtocolAmounts)
    public payable whenNotPaused whenITokenPriceHasNotDecreased
    returns (bool) {
  ...
  function _mintProtocolTokens(address _wrapperAddr, uint256 _amount)
    ...
    returns (uint256 tokens) {
      ...
      address(uint160(_wrapperAddr)).sendValue(_amount);
   ...
   * @return tokens : new tokens minted
   */
  function _redeemProtocolTokens(address _wrapperAddr, address _token, uint256 _amount, address payable _account)
    internal
    returns (uint256 tokens) {
      if (_amount == 0) {
        return tokens;
